...
!developmentChains.includes(network.name)
    ...
    : describe("Nft Marketplace unit tests", () => {
          ...
          beforeEach(async () => {
              ...
              deployer = accounts[0]
              user = accounts[1]
              await deployments.fixture(["all"]) //We deploy all the contracts on the deploy folder
              nftMarketplaceContract = await ethers.getContract("NftMarketplace")
              nftMarketplace = nftMarketplaceContract.connect(deployer)
              basicNftContract = await ethers.getContract("BasicNft")
              basicNft = await basicNftContract.connect(deployer)
              await basicNft.mintNft()
              await basicNft.approve(nftMarketplaceContract.address, TOKEN_ID) //Deployer approve to send it to the marketplace when someone buys it
          })
